...
/* Header - Clean with animated border */
...
.n8n-manager .header-progress {
    ...
    animation: headerBorderFill 0.8s ease-out forwards;
}
@keyframes headerBorderFill {
    0% { width: 0%; }
    100% { width: 100%; }
}
...
.n8n-manager .status-running { background: #d1fae5; color: #065f46; }
.n8n-manager .status-stopped { background: #fee2e2; color: #991b1b; }
.n8n-manager .status-restarting { background: #fef3c7; color: #92400e; }
.n8n-manager .status-unknown { background: #e5e7eb; color: #374151; }

/* Tables */
.n8n-manager .resource-table { width: 100%; font-size: 14px; }
.n8n-manager .resource-table td { padding: 12px 15px; border-bottom: 1px solid #e5e7eb; }
.n8n-manager .resource-table td:first-child { font-weight: 600; width: 120px; color: #64748b; }
...
.n8n-manager .backup-table th, .n8n-manager .backup-table td { padding: 12px 15px; border: 1px solid #e5e7eb; text-align: left; }
.n8n-manager .backup-table th { background: #f8fafc; font-weight: 600; color: #475569; }
.n8n-manager .backup-table tr:hover { background: #f8fafc; }
...
.n8n-manager .info-card table td { padding: 8px 12px; font-size: 14px; text-align: left; word-break: break-word; }
.n8n-manager .info-card p.text-muted { color: #64748b; margin-bottom: 15px; text-align: left; font-size: 13px; }
...
.n8n-manager .form-control:focus {
    border-color: var(--primary);
    box-shadow: 0 0 0 3px rgba(14,80,119,0.1);
    outline: none;
}
...
/* Service Logs Box - Light Theme with Pink Accents */
...
.n8n-manager .confirm-modal h4 { margin: 0 0 15px; font-size: 18px; color: #1e293b; }
.n8n-manager .confirm-modal p { margin: 0 0 25px; color: #64748b; font-size: 14px; line-height: 1.6; }
...
.n8n-manager .success-message { font-size: 18px; color: #1e293b; font-weight: 600; margin-bottom: 5px; }
.n8n-manager .success-detail { font-size: 14px; color: #64748b; }
...
.n8n-manager .toast-message { flex: 1; font-size: 14px; color: #1e293b; }
.n8n-manager .toast-close { cursor: pointer; color: #94a3b8; font-size: 18px; }
.n8n-manager .toast-close:hover { color: #475569; }
@keyframes slideIn {
...
.n8n-manager .refresh-btn:hover { color: var(--primary); background: var(--primary-soft); }
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane active" id="n8n-overview">
            <div class="info-row">
                ...
                <span><strong>Status:</strong>&nbsp;<span id="status-badge" class="status-badge status-unknown">Loading...</span><span class="status-light" id="status-light"></span></span>
            </div>
        ...
        *}
        ...
        <div class="tab-pane" id="n8n-workflows">
            ...
            <div class="info-card">
                ...
                <div class="btn-row" style="margin-top: 10px;">
                    ...
                    <button class="btn btn-default" onclick="loadWorkflowExports()"><i class="fas fa-sync"></i> Refresh List</button>
                </div>
        ...
        <div class="tab-pane" id="n8n-logs">
            ...
            <div class="btn-row" style="margin-bottom:15px;">
                ...
                <button class="btn btn-default" onclick="clearLogs()"><i class="fas fa-trash"></i> Clear Display</button>
            </div>
            ...
            <div class="btn-row" style="margin-bottom:15px;">
                ...
                <button class="btn btn-default" onclick="clearServiceLogs()"><i class="fas fa-trash"></i> Clear Display</button>
            </div>
        ...
        <div class="tab-pane" id="n8n-settings">            
            ...
            <div class="info-card" style="border-left: 3px solid #dc3545;">
                ...
                <div style="display: flex; gap: 15px; flex-wrap: wrap; margin-top: 15px;">
                    <!-- Soft Reset -->
                    <div style="flex: 1; min-width: 250px; padding: 15px; background: #fff8e1; border-radius: 8px; border: 1px solid #ffe082;">
                        <h6 style="margin: 0 0 8px 0; color: #f57c00;"><i class="fas fa-user-times"></i> Soft Reset</h6>
                        <p style="font-size: 12px; color: #6d4c41; margin-bottom: 12px;">Delete all <strong>user accounts</strong> but <strong>keep workflows & credentials</strong>. Create a new account after reset.</p>
                        <button class="btn btn-warning btn-sm" onclick="reinstallSoft()"><i class="fas fa-user-times"></i> Soft Reset</button>
                    </div>
                    
                    <!-- Full Reset -->
                    <div style="flex: 1; min-width: 250px; padding: 15px; background: #ffebee; border-radius: 8px; border: 1px solid #ef9a9a;">
                        <h6 style="margin: 0 0 8px 0; color: #c62828;"><i class="fas fa-trash-alt"></i> Full Reset</h6>
                        <p style="font-size: 12px; color: #6d4c41; margin-bottom: 12px;"><strong>Delete all data</strong> and create a completely fresh N8N instance.</p>
    ...
    fetch(apiUrl + '?action=getAllData&serviceId=' + serviceId)
        ...
        .then(function(d) {
            if (d.success) {
                ...
                dataCache.resourceStats = d.resourceStats;
                dataCache.sslStatus = d.sslStatus;
                dataCache.ipWhitelist = d.ipWhitelist;
                dataCache.versions = d.versions;
                dataCache.workflowExports = d.workflowExports;
                ...
                if (d.workflowExports) renderWorkflowExports(d.workflowExports);
            }
        })
        .catch(function(e) {
            // Production
...
function applyWorkflowExports() {
    if (dataCache.loaded && dataCache.workflowExports) {
...
function applySecurityData() {
    if (dataCache.loaded) {
        if (dataCache.sslStatus) renderSSLStatus(dataCache.sslStatus);
        if (dataCache.ipWhitelist) renderIPWhitelist(dataCache.ipWhitelist);
    } else {
        loadSSLStatus();
        loadIPWhitelist();
    }
    ...
    if (data && data.success && data.exports && data.exports.length > 0) {
        ...
        data.exports.forEach(function(exp) {
            html += '<tr>' +
                ...
                '<td>' + exp.date + '</td>' +
                '<td>' +
                '<a href="modules/servers/dockern8n/download_export.php?serviceid=' + serviceId + '&filename=' + encodeURIComponent(exp.filename) + '" class="btn btn-sm btn-primary"><i class="fas fa-download"></i></a> ' +
                '<button class="btn btn-sm btn-success" onclick="restoreWorkflowFromExport(\'' + exp.filename + '\')"><i class="fas fa-undo"></i></button> ' +
                '<button class="btn btn-sm btn-danger" onclick="deleteWorkflowExport(\'' + exp.filename + '\')"><i class="fas fa-trash"></i></button>' +
                '</td></tr>';
        });
        html += '</tbody></table>';
            ...
            if (d.success && d.exports && d.exports.length > 0) {
                ...
                d.exports.forEach(function(e) {
                    html += '<tr><td>' + e.filename + '</td><td>' + e.size + '</td><td>' + e.date + '</td><td>' +
                        '<a href="modules/servers/dockern8n/download_export.php?serviceId=' + serviceId + '&filename=' + encodeURIComponent(e.filename) + '" class="btn btn-success btn-xs" title="Download"><i class="fas fa-download"></i></a> ' +
                        '<button class="btn btn-primary btn-xs" onclick="restoreWorkflowFromExport(\'' + e.filename + '\')" title="Restore"><i class="fas fa-undo"></i></button> ' +
                        '<button class="btn btn-danger btn-xs" onclick="deleteWorkflowExport(\'' + e.filename + '\')" title="Delete"><i class="fas fa-trash"></i></button>' +
                        '</td></tr>';
                });
                html += '</tbody></table>';
                list.innerHTML = html;
...
function handleTabShown(target) {
    ...
    if (target === '#n8n-resources') {
        ...
        if (!resourceRefreshInterval) {
            resourceRefreshInterval = setInterval(loadResourceStats, 3000);
        }
